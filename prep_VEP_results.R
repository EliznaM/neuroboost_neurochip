...
VEP_results %>% 
  select(X.Uploaded_variation, Location) %>% 
  distinct() %>% 
  filter(!str_detect(Location, "CHR"))

# this doesn't lose any of the original rsIDs
...
# remove the lines for these IDs with only dashes
...
x <- paste0("xx", 1:x)
# x <- c(paste0(letters[1:26], 1), paste0(letters[1:8], 2))
...
write_rds(pheno_tables, "pheno_tables.rds")  
...
keep <- c("X.Uploaded_variation" , "Location" ,"Allele" ,"Consequence"          
          ,"IMPACT" ,"SYMBOL","Gene"                          
          ,"BIOTYPE","Amino_acids","Codons"     
          ,"STRAND" ,"SYMBOL_SOURCE"        
          ,"HGNC_ID" ,"SIFT"  ,"PolyPhen" ,"AF"                   
...
write_rds(VEP_results, "VEP_results.rds")
write_rds(VEP_results_pheno, "VEP_results_pheno.rds")
